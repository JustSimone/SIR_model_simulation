...
sf::Vector2f Balls::random_speed() {
  ...
  } while ((random_speed.x * random_speed.x) +
               (random_speed.y * random_speed.y) <
           1);
  return random_speed;
}
...
void Balls::change_speed(sf::RenderWindow &window, int const seed) {
  sf::Vector2u window_size{window.getSize()};
  int N = ball.size();
  for (int i = 0; i < N; ++i) {
    sf::Vector2u ball_position{ball[i].b.getPosition()};
    sf::Vector2f velocity{perlin_speed(window_size, ball_position, seed)};
...
void Balls::move_balls(sf::RenderWindow &window) {
  int N = ball.size();
  for (int i = 0; i < N; ++i) {
    sf::Vector2u current_position{ball[i].b.getPosition()};
  ...
  for (int i = 0; i < N; ++i) {
    ...
    switch (condition) {
      ...
      case (State::Inf):
        ...
        ball[i].b.setOutlineColor(sf::Color(255, 0, 0, 50));
        break;
      default:
